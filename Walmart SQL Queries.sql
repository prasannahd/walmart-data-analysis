...
Product Analysis
-- 1.How many unique product lines are there in the dataset?
...
-- 5.What month had the  Cost of Goods Sold (COGS)?
...
-- 6.What product line incurred the highest VAT?
SELECT product_line, SUM(vat) as VAT 
FROM sales GROUP BY product_line ORDER BY VAT DESC LIMIT 1;

-- 7.Fetch each product line and add a column product_category, indicating 'Good' or 'Bad,'based on whether its sales are above the average.
